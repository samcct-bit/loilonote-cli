...
function safeHandler<T extends (...args: any[]) => Promise<{ content: any[] }>>(
  ...
  return (async (...args: any[]) => {
    try {
      ...
      return {
        content: [{ type: 'text' as const, text: `❌ 操作失敗：${message}` }],
        isError: true,
      };
    }
  }) as T;
...
server.registerResource(
  'loilonote_courses',
  'loilonote://courses',
  { title: '課程列表', description: '所有 Loilonote 課程與班級群組（JSON 格式）', mimeType: 'application/json' },
  async (uri) => {
    const client = getClient();
...
// --- Resource: 課程筆記列表 ---
server.registerResource(
  'loilonote_course_notes',
  new ResourceTemplate('loilonote://courses/{courseId}/notes', { list: undefined }),
  { title: '課程筆記列表', description: '特定課程內的所有筆記清單（JSON 格式）', mimeType: 'application/json' },
  async (uri, variables) => {
    const client = getClient();
...
// --- Resource: 單篇筆記內容 ---
server.registerResource(
  'loilonote_note_detail',
  new ResourceTemplate('loilonote://courses/{courseId}/notes/{noteId}', { list: undefined }),
  { title: '單篇筆記內容', description: '讀取特定筆記的完整結構與所有純文字內容（JSON 格式）', mimeType: 'application/json' },
  async (uri, variables) => {
    const client = getClient();
    const courseIdStr = String(variables?.courseId || '');
...
// --- Resource: 課程繳交作業匣 ---
server.registerResource(
  'loilonote_course_submissions',
  new ResourceTemplate('loilonote://courses/{courseId}/submissions', { list: undefined }),
  { title: '課程繳交作業匣', description: '特定課程內的所有作業箱清單（JSON 格式）', mimeType: 'application/json' },
  async (uri, variables) => {
    const client = getClient();
    const courseIdStr = String(variables?.courseId || '');
    const courseId = await resolveCourseId(client, courseIdStr);
...
server.registerResource(
  'loilonote_course_users',
  new ResourceTemplate('loilonote://courses/{courseId}/users', { list: undefined }),
  { title: '班級成員名單 (去識別化)', description: '讀取特定課程內的學生名單，且姓名已自動去識別化轉換為 stuXX 格式，保護個資。', mimeType: 'application/json' },
  async (uri, variables) => {
    const client = getClient();
    const courseIdStr = String(variables?.courseId || '');
    const courseId = await resolveCourseId(client, courseIdStr);
