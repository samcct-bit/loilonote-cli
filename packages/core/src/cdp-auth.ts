...
const WebSocketConstructor = (globalThis as unknown as { WebSocket?: typeof WebSocket }).WebSocket;
...
function startChrome(port: number): ReturnType<typeof spawn> {
      ...
      ws.onmessage = (event: MessageEvent) => {
        try {
          ...
          if (msg.method === 'Network.requestWillBeSent') {
            const req = (msg.params as Record<string, unknown>)?.request as Record<string, string> | undefined;
            const m = req?.url?.match(/auth_token=([A-Za-z0-9_-]{10,})/);
            if (m) { stop(); resolve(m[1]); return; }
          }
